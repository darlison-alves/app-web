...
export class AuthService {
  ...
  private isAuthentication$: Rx.BehaviorSubject<boolean>;
  ...
  constructor(private api: ApiService, private tokenService: TokenService) {
    ...
    this.isAuthentication$ = new Rx.BehaviorSubject(false);
  ...
  public login(email: string, password: string): Rx.Observable<void> {
    return this.api
      .post('/auth/login', { email, password, deviceId: navigator.appCodeName, deviceName: navigator.appName })
      .pipe(
        RxOp.tap(() => {
  ...
  public isPageAuthenticated() {
    return this.isAuthentication$.asObservable();
  }
